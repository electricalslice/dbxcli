...
# Setup wizard
db_configure() {
  # Show this only if the user has not already configured the script with app and secret keys
  # We only want to obtain the access token from a URL that is currently not possible for automation
  if [[ -z "${OAUTH_APP_KEY}" && -z "${OAUTH_APP_SECRET}" ]]; then

    echo -ne "\n If this is the first time you run this script, please follow the instructions:\n\n"
    ...
    echo -ne " 5) Now the new configuration is opened, switch to tab \"permissions\" and check \"files.metadata.read/write\", \"files.content.read/write\" and \"sharing.read/write\" \n"
    echo -ne " Now, click on the \"Submit\" button.\n\n"
    echo -ne " 7) Now to tab \"settings\" and provide the following information:\n"
    ...
    echo -ne "Using previous configured app key and secret: \n"
  ...
  echo "   The configuration has been saved. You can now run '${SCRIPT_NAME} test' to check the token permission"
...
if [[ -e $CONFIG_FILE ]]; then
  ...
  if [[ $? -eq 0 ]]; then
    ...
    echo -ne "The old configuration file has been backed up to $CONFIG_FILE.old\n"
    echo -ne "\nDo you want to run the setup wizard? [y/N]: "
    read -r answer
    if [[ $answer == "y" ]]; then
      OAUTH_APP_KEY=""
...
#NEW SETUP...
  ...
  echo -ne "Error, no config file found\n"
  echo -ne "\nDo you want to run the setup wizard? [y/N]: "
  read -r answer
  if [[ $answer == "y" ]]; then
    OAUTH_APP_KEY=""
...
# This will remove dropbox authentication
unlink)

This project was forked from https://github.com/andreafabrizi/Dropbox-Uploader and I have made the following extra improvements and features:

1) Fixed the code global and local variable scopes, remove unnecessary function keywords for all functions, and fixed return codes that cause bugs in API response.
2) Fixed when invalid configuration file is saved, it will have prompt loop bug.
3) The command unlink will now have a question to run setup wizard instead of just removing the authentication and does nothing.
4) The script will now check for the existence of the config file and will ask to run setup wizard if the config file is not found or config file has invalid config (eg: invalid character)
5) Added the new commands:
...
```
wget https://raw.githubusercontent.com/sofibox/dbxcli/master/install.sh
chmod +x install.sh
./install.sh
```
